      ...
      <div className="mx-auto w-full max-w-2xl lg:max-w-7xl">
        ...
        <div className="mb-16 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex flex-col gap-3">
            ...
            <h2 className="max-w-lg text-3xl leading-tight font-black text-neutral-800 lg:text-4xl xl:text-5xl">
              Quatro frentes.{' '}
              <span className="text-rose-600">Um objetivo.</span>
            </h2>
          </div>
        ...
        <div className="grid grid-cols-1 gap-px border border-neutral-100 bg-neutral-100 md:grid-cols-2 lg:grid-cols-4">
          {CAUSES.map(({ number, icon: Icon, title, description, reach }) => (
            <div
              className="group relative flex flex-col gap-6 bg-white p-8 transition-all duration-300 hover:bg-rose-50"
              key={title}
            >
              {/* Giant faded number */}
              <span className="absolute top-4 right-4 text-6xl font-black text-neutral-100 transition-colors duration-300 select-none group-hover:text-rose-100">
                {number}
              </span>

              <div className="flex h-12 w-12 items-center justify-center border-2 border-rose-100 bg-rose-50 transition-colors duration-300 group-hover:border-rose-600 group-hover:bg-rose-600">
                <Icon
